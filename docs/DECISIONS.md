...
重规划：degrade 后再次进入 Planner 时附带 `last_tool_error` 与 `current_step`，由 LLM 决定是否跳过失败工具（不在 degrade 里硬编码跳过）。
...
决策：工具失败分两档，措辞上区分「重试」与「重规划」。
- 同一步失败：retry_count++，路由回 executor 再跑同一步（最多 3 次），不改 plan。
- 重试耗尽：degrade → Planner，带 last_tool_error 重新出 plan；replan_count 达 2 则 plan 置空终止。
- 是否在重规划时跳过某工具：由 Planner LLM 根据依赖关系决定，不在 degrade 里写死跳过逻辑。
原因：简历/面试表述须诚实——不是「失败就重规划」，而是「先重试、再重出计划」。
追问准备：和旧版「degrade 跳过 T4」的区别？→ 当前实现统一交 Planner，避免代码里硬编码跳过规则。
